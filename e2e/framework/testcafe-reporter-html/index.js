...
module.exports = () => ({
  ...
  reportTestDone: function reportTestDone(name, testRunInfo) {
    ...
    const result = hasErr ? 'failed' : testRunInfo.unstable ? 'passed (unstable)' : 'passed';
    ...
    if (hasErr || testRunInfo.unstable) {
      this.compileErrors(name, testRunInfo, result);
    }

    this.currentTestNumber += 1;
  },

  compileErrors: function compileErrors(name, testRunInfo, result) {
    const heading = `${this.currentTestNumber}. ${result} - ${this.currentFixtureName} - ${name}`;
  ...
  compileTestTable: function compileTestTable(name, testRunInfo, hasErr, result) {
    if (hasErr) {
      ...
      this.tableReports += this.indentString('<tr class="info">\n');
...
`;
    ...
    html += `
      ...
      <div class="bg-primary" style="padding:15px;box-shadow: 6px 6px 6px 6px #00000033;border-radius: 10px;">
        ...
        <p class="lead">Tests Unstable: ${this.unstable}</p>
